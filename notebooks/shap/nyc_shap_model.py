...
csv_path = os.path.join(OUT_DIR, "nyc_shap_importance.csv")
...
summary_path = os.path.join(OUT_DIR, f"nyc_shap_summary_pretty_top{TOP_N}.png")
bar_path     = os.path.join(OUT_DIR, f"nyc_shap_bar_pretty_top{TOP_N}.png")

plot_shap_summary_pretty(shap_matrix, X_exp, f"NYC — SHAP Summary (Top {TOP_N} drivers)", max_display=TOP_N)
...
plot_shap_bar_pretty(shap_matrix, X_exp, f"NYC — Global Feature Importance (mean |SHAP|, Top {TOP_N})", max_display=TOP_N)
save_fig(bar_path)
...
for f in top2:
    dep_path = os.path.join(OUT_DIR, f"nyc_dependence_{f}.png")
    plot_shap_dependence_pretty(shap_matrix, X_exp, f, f"NYC — SHAP Dependence: {f}")
    save_fig(dep_path)
...
plt.title(f"NYC — Waterfall Explanation (High-risk case, p={p:.3f})")

wf_path = os.path.join(OUT_DIR, "nyc_shap_waterfall_highrisk.png")
save_fig(wf_path)
...
parquet_path = os.path.join(OUT_DIR, "nyc_shap_values_sample.parquet")
out_pdf.to_parquet(parquet_path, index=False)

notes_path = os.path.join(OUT_DIR, "nyc_shap_notes.md")
top5 = imp.head(5)

lines = []
lines.append("# NYC — SHAP Interpretation Notes\n")
...
Now NYC has professional plots + ranking + reusable SHAP artifacts saved in outputs/.
""")
