...
GRAPH_DIR = os.path.join(BASE_OUTPUT_DIR, "graphs")

os.makedirs(TABLE_DIR, exist_ok=True)
os.makedirs(GRAPH_DIR, exist_ok=True)

ROC_FILENAME = f"rq3_{TRAIN_CITY.lower()}_to_{TEST_CITY.lower()}_roc_curve.png"
ROC_CSV_FILENAME = f"rq3_{TRAIN_CITY.lower()}_to_{TEST_CITY.lower()}_roc_points_all_models.csv"
RESULTS_CSV_FILENAME = f"rq3_{TRAIN_CITY.lower()}_to_{TEST_CITY.lower()}_model_comparison.csv"

ROC_PATH = os.path.join(GRAPH_DIR, ROC_FILENAME)
ROC_CSV_PATH = os.path.join(TABLE_DIR, ROC_CSV_FILENAME)
RESULTS_CSV_PATH = os.path.join(TABLE_DIR, RESULTS_CSV_FILENAME)
...
results_pd["run_ts"] = pd.Timestamp.now()

print("\nFinal summary table:")
display(results_pd)

# Save model comparison CSV
results_pd.to_csv(RESULTS_CSV_PATH, index=False)
print(f"Model comparison CSV saved to: {RESULTS_CSV_PATH}")

# =========================================================
# 4) Save ROC points + Plot ROC curve for all models
# =========================================================
if len(roc_points_dfs) > 0:
    roc_plot_df = pd.concat(roc_points_dfs, ignore_index=True)

    # Save ROC points CSV
    roc_plot_df.to_csv(ROC_CSV_PATH, index=False)
    print(f"ROC points CSV saved to: {ROC_CSV_PATH}")

    plt.figure(figsize=(8, 6))

    for model_name in roc_plot_df["model_name"].unique():
        subset = roc_plot_df[roc_plot_df["model_name"] == model_name].sort_values(["fpr", "tpr"])
        auc_val = next(
    ...
    print("No ROC points were generated, so ROC CSV and plot were not created.")
