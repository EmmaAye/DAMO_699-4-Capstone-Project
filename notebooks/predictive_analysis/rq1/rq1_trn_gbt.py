...
try:
    ...
    metric_path = f"{output_dir}/tables/rq1_trn_gbt_metrics.csv"
    ...
    roc_values_path = f"{output_dir}/graphs/rq1_trn_gbt_roc_points.csv"
    roc_values_df.to_csv(roc_values_path, index=False)
    print(f"ROC values written to {roc_values_path}")

    plt.figure(figsize=(8, 6))
    plt.plot(fpr, tpr, label=f"GBT (AUC = {calc_auc:.3f})")
    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("Toronto ROC Curve - GBT")
    plt.legend(loc="lower right")
    plt.tight_layout()
    plt.savefig(f"{output_dir}/graphs/rq1_trn_gbt_roc_curve.png", dpi=300)
    plt.close()
